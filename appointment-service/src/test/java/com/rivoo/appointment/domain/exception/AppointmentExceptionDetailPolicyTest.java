 ...
 * would be vacuous.
 * <p>
 * appointment-service gets this guard because it has a real anonymous surface
 * ({@code POST /api/v1/appointments/book}, {@code GET /api/v1/appointments/public/**}), so
 * a wrong decision here is publicly visible. The two {@code false} entries for
 * AppointmentConflictException and AppointmentLimitExceededException are the two confirmed
 * leaks this change closed.
 */
class AppointmentExceptionDetailPolicyTest {
    ...
    private static final Map<String, Boolean> EXPECTED = new TreeMap<>(Map.of(
            "AppointmentConflictException", false,
            "AppointmentLimitExceededException", false,
            "AppointmentNotFoundException", true,
            "InvalidStatusTransitionException", true,
            "SalonNotFoundException", false,
            "SalonServiceUnavailableException", false));
    ...
    private static Map<String, Boolean> scanSubtypes() {
        ...
        for (BeanDefinition definition : scanner.findCandidateComponents("com.rivoo.appointment")) {
