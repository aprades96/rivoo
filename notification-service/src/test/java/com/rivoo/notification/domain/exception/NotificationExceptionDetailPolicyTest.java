 ...
 * would be vacuous.
 * <p>
 * notification-service exposes only {@code /api/internal/**} (PSK-gated by
 * {@code InternalEndpointFilter}) and inherits {@code anyRequest().authenticated()} for
 * everything else, so it has NO anonymous surface.
 */
class NotificationExceptionDetailPolicyTest {
    ...
    private static final Map<String, Boolean> EXPECTED = new TreeMap<>(Map.of(
            "NotificationNotFoundException", true));
    ...
    private static Map<String, Boolean> scanSubtypes() {
        ...
        for (BeanDefinition definition : scanner.findCandidateComponents("com.rivoo.notification")) {
