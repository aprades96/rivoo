...
public class EmployeeWorkingHours {
    ...
    /**
     * Reached only from {@code PUT /api/v1/staff/employees/{id}/working-hours},
     * {@code hasRole('SALON_OWNER')}, so every message below is published to the caller via
     * {@link BusinessValidationException#clientSafe(String)}: each one describes the schedule the
     * owner just submitted for their own employee, names nothing else, and is the only thing
     * telling them which row to fix.
     */
