 ...
 * would be vacuous.
 * <p>
 * staff-service declares no security config of its own, so it inherits rivoo-common's
 * {@code anyRequest().authenticated()} and has NO anonymous surface: every subtype here is
 * authenticated-only and every entry below is therefore {@code true}. The guard exists so
 * that stays a checked fact - the day this service gains a {@code permitAll} route, whoever
 * adds it has to come back here and justify each entry instead of inheriting a stale answer.
 */
class StaffExceptionDetailPolicyTest {
    ...
    private static final Map<String, Boolean> EXPECTED = new TreeMap<>(Map.of(
            "AuthServiceException", true,
            "DuplicateServiceNameException", true,
            "EmployeeLimitExceededException", true,
            "EmployeeNotFoundException", true,
            "ServiceOfferingNotFoundException", true));
    ...
    private static Map<String, Boolean> scanSubtypes() {
        ...
        for (BeanDefinition definition : scanner.findCandidateComponents("com.rivoo.staff")) {
