 ...
 * would be vacuous.
 * <p>
 * client-service declares no security config of its own, so it inherits rivoo-common's
 * {@code anyRequest().authenticated()} and has NO anonymous surface. Everything here holds
 * personal data under GDPR, which is exactly why the absence of a {@code permitAll} route
 * must not stay an assumption anyone re-derives by hand.
 */
class ClientExceptionDetailPolicyTest {
    ...
    private static final Map<String, Boolean> EXPECTED = new TreeMap<>(Map.of(
            "ClientAlreadyAnonymizedException", true,
            "ClientNotFoundException", true,
            "DuplicateClientEmailException", true));
    ...
    private static Map<String, Boolean> scanSubtypes() {
        ...
        for (BeanDefinition definition : scanner.findCandidateComponents("com.rivoo.client")) {
