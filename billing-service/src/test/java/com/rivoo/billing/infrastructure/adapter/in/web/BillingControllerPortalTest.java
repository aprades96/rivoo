...
class BillingControllerPortalTest {
    ...
    @Test
    void createPortalSession_postWithNoBody_returns200AndUrlField() throws Exception {
        when(billingPortalUseCase.createPortalSession(TENANT_ID)).thenReturn(new PortalResponse(PORTAL_URL));

        // billing.ts posts with neither body nor Content-Type. A @RequestBody parameter on the
        // handler would turn this exact request into a 400 before the use case is ever reached.
